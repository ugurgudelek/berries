...
if __name__ == "__main__":
    ...
    EXPERIMENT_NAME = 'energy_fcnn_d06'
    # EXPERIMENT_NAME = 'logistic_regression_10feature'
    seed = 7
    np.random.seed(seed)
    random.seed(seed)
    ...
    # dataset = VibrationDataset(train_ratio=train_ratio, kind='acc', shuffle_mode=1)
    ...
    for epoch in range(num_epochs):
        ...
        for data in test_dataloader:
            img, label = data[0].to(device).float(), data[1].to(device).float()
            output = model(img)
            loss = criterion(output, label)
            optimizer.zero_grad()
