...
G_pos, G_neg, B_pos, B_neg, E, s, b = [[0 for n in range(N)], [0 for n in range(N)], [0 for n in range(N)],
                                       [0 for n in range(N)], [0 for n in range(N)], [0 for n in range(N)],
                                       [0 for n in range(N)]]
...
def Optimize(p, PV, L, strategy=True, j=0):
    ...
    for n in range(N):
        G_pos[n] = m.addVar(lb=0,
                            vtype=GRB.CONTINUOUS)  # Decision variables for power flow (kW) drawn from the grid (bought)
        G_neg[n] = m.addVar(lb=0,
                            vtype=GRB.CONTINUOUS)  # Decision variables for power flow (kW) fed into the grid (sold)
        B_pos[n] = m.addVar(lb=0,
                            vtype=GRB.CONTINUOUS)  # Decision variables for power flow (kW) from the battery (discharge)
        B_neg[n] = m.addVar(lb=0,
                            vtype=GRB.CONTINUOUS)  # Decision variables for power flow (kW) into the battery (charge)
        E[n] = m.addVar(lb=0,
                        vtype=GRB.CONTINUOUS)  # Decision variables for instantaneous stored energy (kWh) in the battery (storage)
        s[n] = m.addVar(lb=0,
                        vtype=GRB.CONTINUOUS)  # Pseudo decision variables to represent absolute values of mismatch
        b[n] = m.addVar(lb=0,
                        vtype=GRB.BINARY)  # Pseudo decision variables to prevent buying from and sell into the grid simultaneosly

    # Add constraints to the model
    for n in range(N):
        m.addConstr(G_pos[n] - G_neg[n] + eff * (PV[n] + B_pos[n] - B_neg[n]) - L[
            n] == 0)  # Nanogrid system power flow balance constraints
        if n == 0:
            m.addConstr(E_init - (B_pos[n] - B_neg[n]) * T - E[
                n] == 0)  # Battety energy flow balance constraint (intitial case)
            m.addConstr(G_neg[n] * T <= (eff * E_init + eff * PV[n] * T) * (
                        1 - b[n]))  # Maximum sold energy limit constraints (initial case)
        else:
            m.addConstr(E[n - 1] - (B_pos[n] - B_neg[n]) * T - E[n] == 0)  # Battety energy flow balance constraints
            m.addConstr(G_neg[n] * T <= (eff * E[n - 1] + eff * PV[n] * T) * (
                        1 - b[n]))  # Maximum sold energy limit per period constraints
        m.addConstr(
            eff * G_pos[n] * T <= (E_max + eff * L[n] * T) * b[n])  # Maximum bought energy limit per period constrains
    ...
    if m.status == GRB.status.OPTIMAL:
        ...
        if strategy:
            ...
            result = (p[0] * (G_pos[0].x - m_back * G_neg[0].x) * T) + (c_pen * s[0].x) - (
                        p[j] * E[0].x - p[j - 1] * E_init)
            E_init = E[0].x
        return result
    else:
        print("infeasible")
...
def Write():
    write_all = pd.DataFrame(
        data={"date": Date[:192], "from": From[:192], "to": To[:192], "load": L_list[:192], "pv": PV_list[:192],
