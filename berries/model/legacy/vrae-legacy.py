...
class Encoder(nn.Module):
    ...
    def __init__(self, number_of_features, hidden_size, hidden_layer_depth, latent_length, dropout, block='LSTM'):
        ...
        if block == 'LSTM':
            self.model = nn.LSTM(
                self.number_of_features, self.hidden_size, self.hidden_layer_depth, dropout=dropout)
        elif block == 'GRU':
...
class Decoder(nn.Module):
    ...
    def __init__(self, sequence_length, batch_size, hidden_size, hidden_layer_depth, latent_length, output_size, dtype, block='LSTM'):
        ...
        self.decoder_inputs = torch.zeros(
            self.sequence_length, self.batch_size, 1, requires_grad=True).type(self.dtype)
    ...
    def forward(self, latent):
        ...
        if isinstance(self.model, nn.LSTM):
            ...
            decoder_output, _ = self.model(
                self.decoder_inputs, (h_0, self.c_0))
...
class VRAE(BaseEstimator, nn.Module):
    ...
    def compute_loss(self, X):
        ...
        :param X: Input tensor
        :return: joint loss, reconstruction loss, kl-divergence loss and original input
        """
        x = Variable(X[:, :, :].type(self.dtype), requires_grad=True)

        x_decoded, _ = self(x)
        joint_loss, recon_loss, kl_loss = self._rec(
            x_decoded, x.detach(), self.loss_fn)

        return joint_loss, recon_loss, kl_loss, x
