...
def plot_confusion_matrix(y_true, y_pred, classes, save_path=None, title="Confusion matrix", cmap=plt.get_cmap("Blues"), figsize=None):
    """
    ...
    for i in range(cm.shape[0]):
        for j in range(cm.shape[1]):
            ax.text(j, i, f"{cm[i, j]}\n{cm_norm[i, j]:.2f}", ha="center", va="center", color="white" if cm[i, j] > thresh else "black")
    ...
    with imageio.get_writer(fpath / "animated.gif", mode="I") as writer:
        with tqdm(total=len(filenames), desc="Reading images..") as pbar:
            for filename in filenames:
                ...
                pbar.update(1)
    optimize(source=str(fpath) + "/animated.gif", destination=str(fpath) + "/optimized.gif", colors=10, options=["--verbose"])
...
def plot_clustering(z_run, labels, engine="plotly", title_postfix="z", download=False, folder_name="clustering"):
    ...
    def plot_clustering_matplotlib(z_run, labels, title_postfix, download, folder_name):
        ...
        patches = [mpatches.Patch(color="tab:blue", label="Class 0"), mpatches.Patch(color="tab:orange", label="Class 1"), mpatches.Patch(color="tab:green", label="Class 2")]
        # for _ in np.unique(labels):
        #     hex_colors.append('#%06X' % randint(0, 0xFFFFFF))
    ...
    if engine == "matplotlib":
        plot_clustering_matplotlib(z_run, labels, title_postfix, download, folder_name)


def plot_clustering_legacy(z_run, labels, engine="plotly", download=False, folder_name="clustering"):
    """
    Given latent variables for all timeseries, and output of k-means, run PCA and tSNE on latent vectors and color the points using cluster_labels.
    :param z_run: Latent vectors for all input tensors
    :param labels: Cluster labels for all input tensors
