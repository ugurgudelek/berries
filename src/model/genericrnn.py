...
class RNNPredictor(nn.Module):
    ...
    model = model.RNNPredictor(rnn_type = args.model,   default:LSTM
                           enc_inp_size=feature_dim,
                           rnn_inp_size = args.emsize,  'size of rnn input features', 32
                           rnn_hid_size = args.nhid,    'number of hidden units per layer', 32
                           dec_out_size=feature_dim,
                           nlayers = args.nlayers,      'number of layers', 2
                           dropout = args.dropout,      'dropout applied to layers (0 = no dropout)', 0.2
                           tie_weights= args.tied,      'tie the word embedding and softmax weights (deprecated)'
                           res_connection=args.res_connection) 'residual connection'
