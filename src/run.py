...
def main():
    config = Config()
    dataset = IndicatorDataset(config=config, stock_names=['spy'], label_after=20)
    model = LSTM(input_size=28, seq_length=28, num_layers=1, out_size=1)
    ...
    for epoch in range(20):
        for i, (image, label) in enumerate(train_dataloader):
            ...
            if i % 10 == 0:
                ...
                print(
                    'i: {:<5} | Epoch: {:<3} | Actual: {:015.10f} | Pred: {:015.10f} | TLoss: {:015.10f}'.format(
                        i * label.size()[-1],
                        epoch, loss.data[0],
                        true_val.mean(),
                        pred_val.mean(),
