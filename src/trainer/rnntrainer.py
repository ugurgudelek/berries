...
class RNNTrainer:
    def __init__(self, model, dataset, hyperparams, params, optimizer=None, criterion=None, use_cuda=True):
        ...
        self.loader_kwargs = {'drop_last': True, 'shuffle': True}
    ...
    def _on_epoch(self, epoch, train=True):
        ...
        with with_grad_or_not():           
            ...
            for batch_ix, (data, targets) in enumerate(loader):
                data, targets = data.to(self.device).transpose(0,1), targets.to(self.device).transpose(0,1)
                ...
                loss1 = self.criterion(outSeq1.view(self.hyperparams['train_batch_size'], -1), targets.contiguous().view(self.hyperparams['train_batch_size'], -1))

                '''Loss2: Teacher forcing loss'''
                outSeq2, hidden, hids2 = self.model.forward(data, hidden, return_hiddens=True)
                loss2 = self.criterion(outSeq2.view(self.hyperparams['train_batch_size'], -1), targets.contiguous().view(self.hyperparams['train_batch_size'], -1))

                '''Loss3: Simplified Professor forcing loss'''
                loss3 = self.criterion(hids1.view(self.hyperparams['train_batch_size'], -1), hids2.view(self.hyperparams['train_batch_size'], -1).detach())
    ...
    def fit(self):
        ...
        for epoch in range(1, self.hyperparams['epoch']+1):
            ...
            self.generate_output(self.hyperparams, self.params, epoch, self.model, self.dataset.testset, startPoint=1500)
    ...
    @staticmethod
    def generate_output(hyperparams,params, epoch, model, testset, disp_uncertainty=True, startPoint=500, endPoint=3500):
        if params['save_fig']:
